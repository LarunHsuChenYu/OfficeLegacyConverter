...
- 移除 PDF 最終 `.md` 文末重複的「## 圖片索引」：頁面截圖與描述只在對應頁面輸出一次；尚未內嵌的頁面改插入該頁區塊，不再整份重列。
- 最終 Markdown 統一為 LF（`\n`）換行，避免正文 LF 與索引區塊 CRLF 混用。
- `.raw.md` 仍為原始擷取層，不含頁面截圖備援；本次變更只影響強化後 `.md`。
- Word（OpenXml）路徑若另有圖片索引需求，維持獨立處理，不與 PDF 缺表備援路徑混用。
